...
function _check_royal_r(x,y,r:integer):boolean;
begin
   _check_royal_r:=(dist(map_cx,map_cx,x,y)+r)>g_royal_r;
end;
...
begin
   ...
   begin
      o:=base_r;
      for u:=1 to MaxCapturePoints do
       with g_cpoints[u] do
       begin
   ...
   begin
      d :=dist(dx,dy,tx,ty);
      dr:=d-nr;
      if(0<dr)then _1c_push(@tx,@ty,dx,dy,nr-1);
   end;
    ...
    if(_bldCndt(bp,bt)=false)then
     if(build_b<bx)and(bx<map_b1)and(build_b<by)and(by<map_b1)then
      ...
      begin
         ...
         if(_lcu>0)then
         begin
            cenerg+=_ulst[cl2uid[race,true,bt]].renerg;
            {$IFDEF _FULLGAME}
   ...
   with _units[u] do
    if(rld=0)and(bld)and(ucl=1)and(isbuild)then
     with _players[player] do
      ...
      begin
         if(_ulst[cl2uid[race,false,ut]].max=1)then wbhero+=1;
...
begin
   ...
   begin
      ...
      begin
        ...
        if(uu^.arf>-1)and(tu^.uf=uf_fly)  ///(uu^.uf=uf_ground)and
        then md:=td-uu^.arf
        else md:=td-uu^.ar;
      ...
      for i:=0 to _ubuffs do
       if(buff[i]<_bufinf)then
        ...
        begin
          ...
          if(buff[i]<0)then buff[i]+=1;

      if(onlySVCode)
      or(uid in whocanattack)then
       if(rld>0)then rld-=1;

      if(onlySVCode)and(isbuild)and(rld>2)and(buff[ub_pboost]>0)then
       if(ucl=1)or(ucl=3)then rld-=1;

      for i:=0 to MaxPlayers do
      begin
         if(0<vsnt[i])and(vsnt[i]<_bufinf)then vsnt[i]-=1;
         if(0<vsni[i])and(vsni[i]<_bufinf)then vsni[i]-=1;
      end;
   end;
end;
    ...
    begin
       ...
       begin
          ...
          begin
             ...
             begin
                case isbuild of
                false: if(upgr[upgr_regen ]>0)then hits+=upgr[upgr_regen ];
                true : if(g_addon=false)
                       then hits+=2
                       else
                         if(upgr[upgr_bldrep]>0)then hits+=upgr[upgr_bldrep];    //
                end;
       ...
       case uid of
         ...
         UID_HKeep :  begin
                         ...
                         if(generg<>tt)then
                         begin
                            if(onlySVCode)then menerg+=tt-generg;
                            generg:=tt;
                         end;
                      end;
         ...
         UID_Tank    :if(buff[ub_advanced]>0)
                      then ar:=325
                      else ar:=sr;
         UID_Arachnotron
         ...
         UID_Revenant:if(buff[ub_advanced]>0)
                      then arf:=350
                      else arf:=300;

         UID_Flyer   :if(buff[ub_advanced]>0)
                      then arf:=350
                      else arf:=sr;
         ...
         UID_UPTurret,
         UID_URTurret:begin
                         tt:=towers_sr[mm3(0,upgr[upgr_towers],3)];
                         if(sr<>tt)then
                         begin
                            sr :=tt;
                            ar :=sr;
