...
begin
   if(r^<=0)
   then _wrld:=0
   else _wrld:=min2((r^ div vid_fps)+1,255);
...
begin
   ...
   begin
      ...
      SetBBit(@_bts1,5, (uid in whocanattack)and(tar1>0) ); //and(rld>max2(0,rld_r-vid_3fps))
      SetBBit(@_bts1,6, sel                 );
               ...
               begin
                  ...
                  begin
                     if(uid in clnet_rld)then
                      if(_wrld(@rld,rpl)>0)and(uid in clnet_utr)then _wudata_byte(utrain,rpl);
                  end;
                  if(sel)then
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  begin
                     if(uid in clnet_rld)then
                      if(_rrld(@rld,rpl)>0)and(uid in clnet_utr)then utrain:=_rudata_byte(rpl,0);
                  end;
                  if(sel)then
                   if(uid in whocanmp)then
                   begin
...
begin
   ...
   if((gstp mod vid_hhfps)=0)then
    if(rpl=false)then
     with _players[_pl] do _rrld(@bld_r,rpl);
