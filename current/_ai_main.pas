...
begin
   ...
   begin
      ...
      or((uid_issmith  )and(pprod_r[i]>0))then
      begin
         ai_isnoprod:=false;
         exit;
      end;
...
begin
   ...
   if(cp<>nil)then
    with cp^ do
    begin
       if(d<(kpNoBuildR+NoBuildRArea))then exit;

       if(d<=kpCaptureR)then exit;
    end;
...
begin
   ...
   if(bt=0)then
    ...
    begin
       with pu^.player^ do
        if(units_uid_e[b0]<b0c)
        then SetBTA:=SetBT(b0)
        else
          if(units_uid_e[b1]<=0)
          then SetBTA:=SetBT(b1)
          else
            if(units_uid_e[b1]<(units_uid_e[b0] div b01c))then SetBTA:=SetBT(b1);
   ...
   with pu^ do
    ...
    begin
       ...
       begin
          ...
          l:=srange-g_uids[aiucl_detect[race]].uid_r;
       end;
    end;
end;
...
begin
   ...
   begin
      ...
      then ai_need_energy:=mm3i(600, ai_builders_count*1250,ai_GeneratorsEnergy)
      else
        if(ai_advanced_bld)
        then ai_need_energy:=mm3i(600,(ai_unitp_cur+ai_upgrp_cur)*650 ,ai_GeneratorsEnergy)
        else ai_need_energy:=mm3i(600,(ai_unitp_cur+ai_upgrp_cur)*1000,ai_GeneratorsEnergy);
    ...
    then BuildTech  (1)
         ...
         if(map_pf_lastZone>1)or(race<>r_hell)or(upgrs_cur[upgr_hell_Recall]>0)
    then BuildSpec2 (ai_maxcount_spec2 );
      ...
      rdir:=ddir*degtorad;
      if(l<0)
      then l:=uid_r+random(srange-uid_r);
...
begin
   ...
   with pu^     do
   with player^ do
   if((ai_armylimit_alive_u+prod_unit_Limit)<ai_maxlimit_blimit)then
   begin
      case uclass of
uprod_smart: begin
...
1..255     : ut:=uclass;
      ...
      up_n:=units_uid_e[ut]+prod_unit_uid[ut];

      if(up_n>=count)then exit;

      case ut of
UID_Mastermind,
UID_Cyberdemon   : up_m:=ai_max_specialist-(units_uid_e[UID_Mastermind]+prod_unit_uid[UID_Mastermind]
                                           +units_uid_e[UID_Cyberdemon]+prod_unit_uid[UID_Cyberdemon]);
UID_APC,
UID_UTransport,
UID_Pain,
UID_BFGMarine,
...
r_hell: begin
        ...
        begin
        MakeUpgr(upgr_hell_PainFactor     ,i);
        MakeUpgr(upgr_hell_EvilEyeR      ,i);
        MakeUpgr(upgr_hell_Regeneration     ,i);
        MakeUpgr(upgr_hell_UnitSightR    ,i);
        MakeUpgr(upgr_hell_DistDamage1  ,i);
        MakeUpgr(upgr_hell_DistDamage2  ,i);
        MakeUpgr(upgr_hell_MeleeDamage   ,i);
        MakeUpgr(upgr_hell_UnitArmor    ,i);
        MakeUpgr(upgr_hell_BuildArmor    ,i);
        MakeUpgr(upgr_hell_Regeneration     ,i);
        MakeUpgr(upgr_hell_UnitSightR    ,i);
        end;
        end;

        MakeUpgr(upgr_hell_DistDamage1+random(21),ai_maxcount_upgrlvl);
        MakeUpgr(upgr_hell_TowerBlink    ,15);
        end;
r_uac : begin
        if((ai_flags and aif_upgr_smart_opening)>0)then
        begin
        //if(map_generators=0)then
        MakeUpgr(upgr_uac_BuilderR     ,2);
        MakeUpgr(upgr_uac_CCFly      ,1);
        MakeUpgr(upgr_uac_CommandoInvis   ,1);
        MakeUpgr(upgr_uac_CCAttack     ,1);
        MakeUpgr(upgr_uac_DronTurret  ,1);
        MakeUpgr(upgr_uac_TerAAWeapon    ,1);

        if(ai_maxcount_upgrlvl>0)then
        for i:=1 to ai_maxcount_upgrlvl do
        begin
        MakeUpgr(upgr_uac_UnitSightR     ,i);
        MakeUpgr(upgr_uac_DistDamage     ,i);
        MakeUpgr(upgr_uac_BioArmor     ,i);
        MakeUpgr(upgr_uac_MechArmor    ,i);
        MakeUpgr(upgr_uac_BuildArmor     ,i);
        MakeUpgr(upgr_uac_RepairTools      ,i);
        MakeUpgr(upgr_uac_MechSpeed    ,i);
        MakeUpgr(upgr_uac_UnitSightR     ,i);
        end;
        end;

        MakeUpgr(upgr_uac_DistDamage+random(21),ai_maxcount_upgrlvl);
        end;
      end;
end;
...
begin
   ...
   begin
      if(energyl_cur<0)then
       if(prod_upgr_Now<=0)and(prod_unit_Now<=0)and(not iscomplete)then exit;
...
UID_UGenerator2,
UID_UGenerator3,
UID_UGenerator4: if(energyl_cur>uid_EnergyGen)and(armylimit>ai_GeneratorsDestoryLimit)and(energyl_max>ai_GeneratorsDestroyEnergy)then exit; // ai_enrg_cur
      else
        if(units_uid_c[uidi]>1)then
         if(uid_isbarrack)or(uid_issmith)then
          if(ai_isnoprod(pu))then
          begin
             i:=level+1;

             if(uid_isbarrack)and(ai_unitp_cur>2)and(units_uid_c[uidi]>1)then
              if(ai_unitp_cur_na<=0)or((level=0)and(ai_unitp_cur_na>0))then
               if((ai_unitp_cur-i-5)>=ai_unitp_need)then exit;
             if(uid_issmith  )and(ai_upgrp_cur>1)then
              if(ai_upgrp_cur_na<=0)or((level=0)and(ai_upgrp_cur_na>0))then
               if((ai_upgrp_cur-i-3)>=ai_upgrp_need)then exit;
          end;
      end;

      if(ai_PhantomWantZombieMe)and(iscomplete)and(hits<=(uid_ZombieHits+BaseDamage4))then exit;

      case uidi of
UID_HTower,
UID_HTotem         : if(ai_need_suicide_tower(pu))then exit;
UID_UGTurret,
UID_UATurret       : if(upgrs_cur[upgr_uac_DronTurret]=0)then
...
UID_UATurret      : if(ai_towers_near_grd=0)then exit;
     ...
     if(ai_enemy_d>base_2r)and(buffs[ub_Damaged]<=0)and(a_rld<=0)and(units_uid_e[uidi]>1)then
      case uidi of
UID_HTower        : if((units_uid_e[uidi]-units_uid_e[UID_HTotem])>1)then exit;
UID_HTotem        : if((units_uid_e[uidi]-units_uid_e[UID_HTower])>1)then exit;
      end;
...
UID_UCommandCenter:
                    begin
                      ...
                      or((race=r_hell)and(u_royal_d>base_5r))then
                        if(ai_inprogress_auid<2)and(ai_inprogress_uid=0)and(units_builders_ec>1)and(ai_enemy_d>base_2r)and(ai_unitp_cur>0)and(ai_enrg_cur>=1800)then exit;
                    end;
UID_HSymbol1,
UID_UGenerator1,
UID_HSymbol2,
UID_UGenerator2,
UID_HSymbol3,
UID_UGenerator3   : if(energyl_cur>=300)or((prod_unit_Max=prod_unit_Now)and(prod_upgr_Max=prod_upgr_Now))then exit;
      else
         if(uid_isbarrack)or(uid_issmith)then
           if(level<1)or((level>=1)and(ai_ReadyForAttack))then
             if((units_uid_e[uidi]-units_uid_c[uidi])<units_uid_c[uidi])then
               if(level<LastUnitLevel)and(energyl_cur>=600)and(ai_isnoprod(pu))then exit;
      end;
   end;
...
begin
   ...
   begin
      ...
      ai_builders_count:=units_uid_e[aiucl_main0 [race]]
                        +units_uid_e[aiucl_main0A[race]]
                        +units_uid_e[aiucl_main1 [race]]
                        +units_uid_e[aiucl_main1A[race]];
      ai_tech0_cur     :=units_uid_e[aiucl_tech0 [race]];
      ai_tech1_cur     :=units_uid_e[aiucl_tech1 [race]];
      ai_tech2_cur     :=units_uid_e[aiucl_tech2 [race]];
      ai_detect_cur    :=units_uid_e[aiucl_detect[race]]*g_uids[aiucl_detect[race]].uid_LimitUse;
      ai_spec1_cur     :=units_uid_e[aiucl_spec1 [race]];
      ai_spec2_cur     :=units_uid_e[aiucl_spec2 [race]];

      if(_N(@ai_builders_need,ai_maxcount_mains ))then ai_builders_need:=ai_builders_count+1;

      prods:=(energyl_max div 500)-1+ai_builders_count+ai_tech1_cur+ai_tech2_cur;
      if(map_generators>0)or((ai_tech2_cur>0)and ai_advanced_bld)then prods+=2;
      ...
      //if(isselected)then writeln(ai_enrg_pot,' ',ai_maxcount_energy,' ',ai_unitp_need,' ',ai_maxcount_unitps);
      ...
      begin
         if(map_generators>0)then
         begin
            ai_towers_need :=min2i(units_bld_e[false],ai_maxcount_towers);
         ...
         begin
            ...
            ai_towers_needx    :=ai_kpoint_kp^.kpx;
            ai_towers_needy    :=ai_kpoint_kp^.kpy;
            ai_towers_need_type:=0;
            ai_towers_needl    :=-1;
         end
         else
         ...
         if(ai_choosen)and(map_scenario=mc_royale)and(u_royal_cd<base_2r)then
         begin
            ai_towers_need:=ai_maxcount_towers;
            ai_towers_need_type:=0;
            ai_towers_needl    :=-1;
      ...
      if(units_builders_ec>0)and(ai_nearest_builder_u<>nil)then ai_builder(ai_nearest_builder_u);
      ...
      if(uid_issmith  )then
        if(energyl_cur<0)and(prod_unit_Now<=0)
...
function ai_TryTeleportF(pTarget,pTBeacon:PTUnit):boolean;
var tt,td:integer;
begin
   ...
   if(ai_teleport_use)and(ai_teleporterF_u<>nil)then
     ...
     begin
        pTarget^.uo_x:=x;
        pTarget^.uo_y:=y;
        td:=point_dist_int(pTarget^.x,pTarget^.y,x,y);
        ai_TryTeleportF:=true;
        if(td<uid^.uid_r)then
          if(pTBeacon<>nil)then
          begin
             tt:=rpoint_tar;
             rpoint_tar:=pTBeacon^.unum;
             unit_ability_teleport(pTarget,ai_teleporterF_u,td);
             rpoint_tar:=tt;
          end
          else
            if(IsUnitRange(rpoint_tar,nil))then unit_ability_teleport(pTarget,ai_teleporterF_u,td);
     end;
end;
...
begin
   ...
   if(ai_teleport_use)and(ai_teleporterR_u<>nil)then
     with ai_teleporterR_u^ do
       if(rld<=0)and(player^.upgrs_cur[upgr_hell_Recall]>0)then
       begin
...
begin
   ...
   with pu^ do
    with uid^ do
     if(a_rld>0)and(a_weap_cl<=LastUnitArms)then
...
UID_Cyberdemon : if(srange<ai_enemy_build_d)and(ai_enemy_build_d<base_1rh)then
   ...
   begin
      if(transportC>0)then
        ...
        or(ai_alarm_d    <200)
        or(ai_enemy_d    <200)then uo_id:=ua_sability;
...
begin
   ...
   begin
      ...
      if(map_scenario=mc_royale)and(u_royal_d<100)then
      begin
...
UID_Medic    : if(CheckReparTargets(ai_urepair_u,ai_urepair_d))then exit;
      ...
      if(ai_generator_d<NOTSET)then
        with ai_generator_cp^ do
          ...
          then SetNearestTarget(nil,kpx,kpy,ai_generator_d,kpzone,false,false,1)
          else SetNearestTarget(nil,kpx,kpy,ai_generator_d,kpzone,ai_generator_d>base_3r,ai_generator_d>base_3r,0);
      ...
      if(ai_transport_tar_d<NOTSET)then
        ...
        begin
           ...
           d:=ai_transport_tar_d-(uid_r+ai_transport_tar_u^.uid^.uid_r-aw_dmelee);
           if(d<=0)
   ...
   begin
      ...
      begin
         ...
         then ai_RunTo(pu,0,map_hSize,map_hSize,0,nil)
           ...
           if(ai_choosen)and(map_scenario=mc_royale)then
           begin
             ...
             begin
                ai_RunTo(pu,ai_kpoint_d,ai_kpoint_kp^.kpx,ai_kpoint_kp^.kpy,base_1r,nil);
               ...
               begin
                  ai_BaseIdle(pu,base_hr+uid^.uid_r);
        ...
        then unit_sability(pu,false)
          ...
          if(not ai_kpoint_koth)or(ai_kpoint_d>base_1r)then
           if(aiu_limitaround_enemy>aiu_limitaround_ally)and(buffs[ub_Damaged]>0)then
            if(hits<uid^.uid_MaxHitsh)or(aiu_limitaround_enemy>ul15)then unit_sability(pu,false);
   end;
end;
...
begin
   ...
   begin
      ...
      or(ai_kpoint_d>base_1r)then
        if(aiu_alarm_d<base_2r)and(ai_builders_count<=ai_MinBaseSaveCountBorder)and(hits<uid^.uid_MaxHitsh)then
        begin
           if(map_scenario=mc_royale)
           then w:=g_royal_r div 2
           else w:=map_hSize;
           if(unit_ability_HKeepBlink(pu,map_hSize+g_random(w),map_hSize+g_random(w),false)=0)then exit;
        end;
...
mc_KotH  : if(ai_choosen)and(base_1r<ai_kpoint_d)and(ai_kpoint_d<NOTSET)and(ai_kpoint_koth)then
           begin
              w:=base_1r;
              unit_ability_HKeepBlink(pu,ai_kpoint_kp^.kpx+g_random(w),ai_kpoint_kp^.kpx+g_random(w),false);
              exit;
           end;
   ...
   begin
      ...
      if(ai_kpoint_d<NOTSET)then
        with ai_kpoint_kp^ do
          if(kpNoBuildR<kpCaptureR)then
            if(ai_kpoint_d<=kpCaptureR)
            then exit
            else SetBlinkTarget(kpx,kpy,ai_kpoint_d,kpzone);

      if(ai_generator_d<NOTSET)then
        with ai_generator_cp^ do
          if(kpNoBuildR<kpCaptureR)then
            if(ai_generator_d<=kpCaptureR)
            then exit
            else SetBlinkTarget(kpx,kpy,ai_generator_d,kpzone);
...
begin
   ...
   begin
      ...
      if(uid_ability_isteleport)then
        ...
        then rpoint_tar:=ai_teleporter_beacon_u^.unum
        else rpoint_tar:=0;
...
uab_SpawnLost        : if(ai_ZombieTarget_d<srange)and(player^.upgrs_cur[upgr_hell_Phantoms]>0)then
