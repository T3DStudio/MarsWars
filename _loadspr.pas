...
const btnhkeys: array[0..2,0..2] of cardinal = ((SDLK_Q,SDLK_W,SDLK_E),
                                                (SDLK_A,SDLK_S,SDLK_D),
                                                (SDLK_Z,SDLK_X,SDLK_C));
var iu,ir,it:byte;
function btn2key(x,y:byte):cardinal;begin if(x<3)and(y<3)then btn2key:=btnhkeys[y,x] else btn2key:=0;end;
begin
   FillChar(ui_UIPBTNS,SizeOf(ui_UIPBTNS),0);
   FillChar(ui_UIABTNS,SizeOf(ui_UIABTNS),0);
       ...
       if(_btnx<=ui_p_lsecwi)and(_btny<=ui_p_btnsh)then
        for ir:=1 to race_n do
         ...
         begin
            ...
            ui_UIPBTNS[_btnx,_btny,it,ir]:=iu;
         end;

      with _toids[iu] do _obtn:=_dsurf;
   end;

   FillChar(ui_UIMBTNS,SizeOf(ui_UIMBTNS),0);
   ...
   for iu:=0 to 255 do
    ...
    begin
       _omarc:=c_white;
       if(iu in [0,uo_upload,uo_prod,uo_uteleport])then _oidi:=uo_move;
       if(iu in [uo_smove,uo_auto])then _oidi:=uo_stop;
       if(iu = uo_spatrol)then _oidi:=uo_patrol;
    end;
